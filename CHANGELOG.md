...
- `caps_check` records `mouse_report_on_enable` and one such row per tracking mode: whether the
  ...
  ghostty was seen on 2026-09-10 to report the pointer's movement under mode 1002 with nothing
  held, which the old steps took for the click or the movement; with the pointer still, no
  terminal answers a mouse enable. The click reading now wants a press, and a motion reading
  wants three reports across its window rather than one.
...
- `caps_check` drains the input before each y/n question and takes only y, n, or q as an answer,
  so a mouse report left over from the tracking steps no longer skips the question; the steps that
  wait for the terminal say how long they will wait and that q skips.
...
[Unreleased]: https://github.com/plambert/termbuf.cr/compare/v0.5.0...HEAD
