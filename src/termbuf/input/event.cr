...
module TermBuf::Input
  ...
  # own `Events::Resize` is one such: it lives on the terminal side because it
  # carries a `ScreenSize`, and it arrives on this channel like the rest.
  #
  ...
  module Events
    ...
    # consumed by the driver, which answers it with `TermBuf::Events::Resize`.
    #
